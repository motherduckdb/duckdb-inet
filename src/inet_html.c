...
static bool decode_codepoint(uint32_t cp, uint32_t *sz, char *c) {
    ...
    if (cp == 0xFFFE || cp == 0xFFFF || cp == 0x1FFFE || cp == 0x1FFFF || cp == 0x2FFFE ||
        cp == 0x2FFFF || cp == 0x3FFFE || cp == 0x3FFFF || cp == 0x4FFFE || cp == 0x4FFFF ||
        cp == 0x5FFFE || cp == 0x5FFFF || cp == 0x6FFFE || cp == 0x6FFFF || cp == 0x7FFFE ||
        cp == 0x7FFFF || cp == 0x8FFFE || cp == 0x8FFFF || cp == 0x9FFFE || cp == 0x9FFFF ||
        cp == 0xAFFFE || cp == 0xAFFFF || cp == 0xBFFFE || cp == 0xBFFFF || cp == 0xCFFFE ||
        cp == 0xCFFFF || cp == 0xDFFFE || cp == 0xDFFFF || cp == 0xEFFFE || cp == 0xEFFFF ||
