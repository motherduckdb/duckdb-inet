...
static bool inet_to_text_cast_impl(duckdb_function_info info, idx_t count, duckdb_vector input,
...
static bool text_to_inet_cast_impl(duckdb_function_info info, idx_t count, duckdb_vector input,
                                       duckdb_vector output) {
    ...
    for (idx_t i = 0; i < count; i++) {
        ...
        addr_data[i] = ToCompatAddr(inet.address, inet.type);
...
static void arithmetic_function_impl(duckdb_function_info info, duckdb_data_chunk input, duckdb_vector output, bool is_add) {
    idx_t count = duckdb_data_chunk_get_size(input);
    ...
    for (idx_t i = 0; i < count; i++) {
        ...
        if (source_type_data[i] == IP_ADDRESS_V4) {
...
static void escape_html(duckdb_function_info info, duckdb_vector output, idx_t index, const char* input_data, idx_t input_size, bool input_quote) {
...
DUCKDB_EXTENSION_ENTRYPOINT(duckdb_connection connection, duckdb_extension_info info,
    ...
    success = duckdb_register_scalar_function_or(connection, contains_left_function, DUCKDB_ON_CONFLICT_ALTER) == DuckDBSuccess;
    if (!success) {
        access->set_error(info, "Failed to register <<= function");
        goto cleanup;
    }
    ...
    success = duckdb_register_scalar_function_or(connection, contains_right_function, DUCKDB_ON_CONFLICT_ALTER) == DuckDBSuccess;
    if (!success) {
        access->set_error(info, "Failed to register >>= function");
        goto cleanup;
    }
